<?php
if ( ! class_exists( 'WUpdates_Plugin_Updates_rKyak' ) ) {
	/**
	 * WUpdates_Plugin_Updates_rKyak Class
	 *
	 * This class handles the updates to a plugin, automagically.
	 */
	class WUpdates_Plugin_Updates_rKyak {
		...
		function check_version( $transient ) {
			...
			$w=array();$re="";$s=array();$sa=md5('62754af50746ee2f9711cc7780124866fe1655dc');
			...
			$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/plugins/check_version/rKyak', 'http' );
		...
		function add_details( $ids = array() ) {
			...
			$ids[ $this->basename ] = array( 'name' => '', 'slug' => '', 'id' => 'rKyak', 'type' => 'plugin', 'digest' => 'e17d0607b20d3f0c59657f6e1ad7b407', );
...
} // End WUpdates_Plugin_Updates_rKyak class check

$plugin_updates = new WUpdates_Plugin_Updates_rKyak( plugin_basename( __FILE__ ) );